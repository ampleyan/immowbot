...
class WorkflowTest(unittest.TestCase):
    ...
    def test_defaults_and_persistence(self):
        ...
        saved = self.store.save_workflow(self.user_id, 'immoweb', '1', {'status': 'Contacted', 'agent_name': 'Alex'})
        self.assertEqual(saved['status'], 'Contacted')
        self.assertEqual(self.store.get_workflow(self.user_id, 'immoweb', '1')['agent_name'], 'Alex')

    def test_invalid_status_rejected(self):
        with self.assertRaises(ValueError):
            self.store.save_workflow(self.user_id, 'immoweb', '1', {'status': 'Unknown'})

    def test_interactions_are_persisted_newest_first(self):
        first = self.store.add_interaction(self.user_id, 'immoweb', '1', 'call', 'Asked about the viewing')
        second = self.store.add_interaction(self.user_id, 'immoweb', '1', 'email', 'Sent available times')

        interactions = self.store.get_interactions(self.user_id, 'immoweb', '1')
    ...
    def test_status_change_creates_interaction(self):
        self.store.save_workflow(self.user_id, 'immoweb', '1', {'status': 'Contacted'})
    ...
    def test_rejection_reason_is_persisted(self):
        saved = self.store.save_workflow(self.user_id, 'immoweb', '1', {'status': 'Rejected', 'rejection_reason': 'Too expensive'})
